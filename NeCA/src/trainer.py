...
def rotation_matrix_to_axis_angle(m, eps=1e-8):
...
class Trainer:
    def __init__(self, cfg, device="cuda"):
        ...
        proj_files = sorted(glob.glob(os.path.join(data['datadir'], "*_proj.npy")))
        assert len(proj_files) > 0, f"No *_proj.npy files found in {data['datadir']}"

        # 加载所有样本，拼接成一个大的 projection batch
        # 每个文件应该是 (1, 3, 512, 512)，拼接成 (N, 3, 512, 512)
        data["projections"] = np.concatenate([np.load(f) for f in proj_files], axis=0)
        ...
        #print(f"[DEBUG] image_reso: {image_reso}")
        ...
        print("========== FIRST PROJECTION DEBUG ==========")
        print(f"image_size  : {image_size}")
        print(f"image_reso  : {image_reso}")
        print(f"proj_size   : {proj_size}")
        print(f"proj_reso   : {proj_reso}")
        print(f"dde         : {dde}")
        print(f"dso         : {dso}")
        print(f"proj_axis   : {proj_axis}")
        print(f"proj_angle  : {proj_angle}")
        print("============================================")
